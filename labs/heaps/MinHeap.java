...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	/**
	 * We create an array with sufficient space
	...
	/**
	 * This method looks at a parent and its two
		...
		if (array[left] != null) {
			if (array[right] != null) {
				if (array[left].getValue().compareTo(array[right].getValue()) <= 0
						&& array[where].getValue().compareTo(array[left].getValue()) > 0) {
					swap(where, left);
					move(where, left);
					heapify(left);					
				}
				
				else if (array[right].getValue().compareTo(array[left].getValue()) < 0
						&& array[where].getValue().compareTo(array[right].getValue()) > 0) {
					swap(where, right);
					move(where, right);
					heapify(right);
				}
			...
			else if (array[where].getValue().compareTo(array[left].getValue()) > 0) {
				swap(where, left);
				move(where, left);
				heapify(left);
			}
	...
	{
		array[first].loc = first;
		array[second].loc = second;
	}
	/**
