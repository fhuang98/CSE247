	...
	public void run() {
		...
		{	
			...
			for (Edge e : current.getVertex().edgesFrom())
				{
					Vertex v = e.to;
					Decreaser<VertexAndDist> target = map.get(v);
					
					if ((dist + weights.get(e)) < target.getValue().getDistance())
					{
						target.decrease(target.getValue().sameVertexNewDistance(weights.get(e)+dist));
						toEdge.put(v, e);
						//ticker.tick();
					}
